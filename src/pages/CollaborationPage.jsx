...
import CollaborationRoom from '../components/editor/CollaborationRoom'
import toast from 'react-hot-toast'
import { Columns2, Eye, Sun, Moon, CornerDownRight, ChevronLeft, Copy, Users } from 'lucide-react'
...
export default function CollaborationPage() {
  ...
  // Generate or read the stable Room ID
  const [generatedRoomId] = useState(() => roomId || `room-${Date.now().toString().slice(-6)}`)
  ...
  useEffect(() => {
    openProject(`proj-collab-${generatedRoomId}`) // Initialize unique team workspace in context
  }, [generatedRoomId])
  ...
  if (!activeProject || !activeProject.id.startsWith('proj-collab-')) {
    return (
      <div style={{ color: '#fff', padding: 40, background: '#0f0f0f', height: '100vh' }}>
        Spinning up Collaborative Coding Environment...
      </div>
    )
  }

  const roomLink = `${window.location.origin}/collaborate/${generatedRoomId}`
  ...
  return (
    <div style={{
      ...
      {/* Sleek Header Bar (Replacing MenuBar dropdown panels) */}
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <span style={{ color: '#333' }}>/</span>
          <span style={{ fontSize: 13, color: '#fff', fontWeight: 600 }}>Collaborative Room</span>
          <span style={{ fontSize: 11, background: '#2c2c3c', color: '#34d399', padding: '2px 8px', borderRadius: 4, fontFamily: 'monospace', marginLeft: 6 }}>
            ID: {generatedRoomId}
          </span>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            {activeProject.members.map(m => (
              <div key={m.id} title={`${m.name} (${m.role})`} style={{
                ...
                background: m.id === 1 ? '#7c6ff7' : '#2b2b3a',
                border: m.id !== 1 ? '1px solid #34d399' : 'none',
          ...
          <button onClick={handleCopyRoomLink} style={{
            background: '#34d399', border: 'none', borderRadius: 6,
            padding: '8px 18px', color: '#000', cursor: 'pointer',
            fontSize: 12, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6,
            transition: 'background 0.2s'
          }}
          onMouseEnter={e => e.currentTarget.style.background = '#2bb281'}
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        {/* Right Panel (Collaboration Room panel) */}
        <div style={{
          width: 350,
          background: '#121217',
